...
//				_helpMatrix.d = 1;
		...
		override protected function generateMesh(textureAtlas:Object, fullName:String, meshData:MeshData):Object 
		{
			...
			if (subTexture)
			{
				...
				var vertexData:starling.rendering.VertexData = new starling.rendering.VertexData();
				
				for (i = 0, l = meshData.numVertex; i < l; i++)
				{
					const dbVertexData:dragonBones.objects.VertexData = meshData.vertices[i];
					vertexData.setPoint(i, "texCoords", dbVertexData.u, dbVertexData.v);
					vertexData.setPoint(i, "position", dbVertexData.x, dbVertexData.y);
				}
				...
				{
					indexData.setIndex(i, meshData.triangles[i]);
				}
